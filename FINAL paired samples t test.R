...
critical_value_lower <- qt(p=.05, df=sample_size-1, lower.tail=TRUE)
critical_value_upper <- qt(p=.05, df=sample_size-1, lower.tail=FALSE)

# defining upper and lower boundaries

upper_x_possible <- mean1+5*sd1
...
if(t>critical_value_upper | t<critical_value_lower) {
  dis_4 <- dis_3 + stat_function(fun=ShadeRegion, geom= "area", fill="red", alpha=0.1)
  dis_5 <- dis_4 + xlab("") + ylab("Probability Density")
